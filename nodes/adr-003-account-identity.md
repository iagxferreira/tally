---
id: 01M1JZNSS9GVH9M87PX34Q1Y91
title: ADR 003 — Account identity
kind: rfc
aliases: [003-account-identity]
originProject: tally
origin: /home/iago/workspace/tally/docs/adr/003-account-identity.md
created: 2026-09-03T06:35:37.129049745Z
updated: 2026-09-03T16:49:23.453597865Z
documentName: 003-account-identity
---
...
The **decision stands**: account identity is a UUIDv7 newtype — minted without
coordination, sorting near-monotonically in an index because the high bits are
a millisecond timestamp.

What changes is that it stops being free. In Rust this was
`uuid = { version = "1", features = ["v7"] }` and one call. In Java:

- `java.util.UUID` can *hold* a v7 value but cannot *mint* one.
  `UUID.randomUUID()` is v4 only, and there is no v7 factory in the JDK as of
  Java 25.
- Minting therefore needs either a library (JUG —
  `com.fasterxml.uuid:java-uuid-generator`, which has a v7 generator) or a
  hand-rolled constructor packing `System.currentTimeMillis()` into the top 48
  bits with the version/variant nibbles set correctly and the remainder from
  `SecureRandom`.

The newtype itself also costs more. Rust's `struct AccountId(Uuid)` was a
zero-cost wrapper; a Java `record AccountId(UUID value)` is a real heap object
unless it becomes a value class. That is an acceptable MVP cost, not a reason
to pass raw `UUID` around — the type distinction is the point.